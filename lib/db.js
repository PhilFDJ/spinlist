...
module.exports = {
  ...
  /* The last track Music Manager reported, with the verdict already worked out.
     Kept on the wedding so the gig window can poll one cheap endpoint instead of
     holding a socket open on venue wifi. */
  setWeddingNowPlaying(weddingId, payload) {
    const w = this.getWedding(weddingId);
    if (!w) return null;
    w.now_playing = payload ? Object.assign({ at: Date.now() }, payload) : null;
    persist();
    return w;
  },
  ...
  /* Matched on email, because that's the one thing a person reliably reuses —
     names get typed three different ways across a year of enquiries. Without an
     email there's nothing safe to match on, so a new record is made rather than
     risk merging two different people. Blanks on an existing record are filled
     in, but nothing already there is overwritten: what the client typed on a
     form shouldn't clobber what the DJ has since corrected by hand. */
  findOrCreateClient(ownerId, c) {
    const email = (c && c.email || '').toString().trim().toLowerCase();
    if (email) {
      const found = (store.clients || []).find(x =>
        x.owner_id === ownerId && (x.email || '').toLowerCase() === email);
      if (found) {
        let touched = false;
        if (!found.name && c.name) { found.name = c.name.toString().slice(0, 120); touched = true; }
        if (!found.phone && c.phone) { found.phone = c.phone.toString().slice(0, 40); touched = true; }
        if (touched) persist();
        return found;
      }
    }
    return this.createClient(ownerId, c || {});
  },
  ...
  getContractByQuote(quoteId) {
    if (!quoteId) return null;
    return (store.contracts || []).find(c => c.quote_id === quoteId && c.status !== 'void') || null;
  },
  ...
  createQuote(ownerId, q) {
    ...
    const quote = {
      ...
      /* The contract template to raise when this quote is accepted. Stored on
         the quote rather than issued alongside it, so declining a quote never
         leaves a stray agreement behind. */
  ...
  updateQuote(id, fields) {
    ...
    if (fields.template_id !== undefined) q.template_id = fields.template_id || null;
  ...
  setWeddingBlockSongs(weddingId, blockId, songs) {
    ...
    /* Carry forward anything the caller didn't send. This mapper rebuilds each
       song from a whitelist, so any field it forgets is destroyed on every save
       — and both the couple's page and the DJ's page save the whole block. That
       silently ate `isrc`, and would have eaten the notes too. */
    const prev = {};
    (block.songs || []).forEach(s => { prev[s.id] = s; });

    block.songs = (Array.isArray(songs) ? songs : []).slice(0, cap).map(s => {
      const old = prev[s.id] || {};
      const keep = (sent, held) => (sent === undefined ? (held || '') : (sent || '').toString().slice(0, 500));
      return {
        id: s.id, uri: s.uri || null, title: s.title, artist: s.artist, art: s.art || '',
        isrc: s.isrc === undefined ? (old.isrc || '') : (s.isrc || ''),
        played: s.played ? 1 : 0,
        // A note each, so the couple and the DJ can't overwrite one another.
        note_couple: keep(s.note_couple, old.note_couple),
        note_dj: keep(s.note_dj, old.note_dj),
      };
    });
    persist();
    return w;
  },
  /* One note at a time, addressed by song id, so adding a note never rewrites
     the playlist — no risk of a stale page reordering songs as a side effect of
     someone typing a comment. `which` is 'couple' or 'dj'; the route decides
     which of those the caller is allowed to be. */
  setWeddingSongNote(weddingId, blockId, songId, which, note) {
    const w = this.getWedding(weddingId);
    if (!w) return null;
    const block = (w.blocks || []).find(b => b.id === blockId);
    ...
    if (!song) return null;
    const field = which === 'dj' ? 'note_dj' : 'note_couple';
    song[field] = (note || '').toString().slice(0, 500);
    persist();
    return w;
  },
  ...
  saveBlockTemplate(userId, tpl) {
    ...
    const clean = {
      ...
      blocks: (Array.isArray(tpl.blocks) ? tpl.blocks : []).slice(0, 30).map(b => ({
        ...
        // Marks this as the block guests can request into. A property of the
        // block, not a magic name — the old check tested for a block literally
        // called "Play If Possible", so renaming it silently broke live requests.
        live: !!b.live,
        /* Songs here must NOT be played. Same reasoning: it was previously just
           a block someone happened to name "Do Not Play", so renaming it — or a
           sub-DJ not knowing the convention — made the ban invisible. */
        noplay: !!b.noplay,
      })),
    };
    /* At most one live-request block per template. Two would leave the gig with
       no clear answer to "where do guest requests land?" — last one wins. */
    let seenLive = false;
    for (let i = clean.blocks.length - 1; i >= 0; i--) {
      if (clean.blocks[i].noplay) clean.blocks[i].live = false;   // can't be both
      if (!clean.blocks[i].live) continue;
      if (seenLive) clean.blocks[i].live = false;
      seenLive = true;
    }

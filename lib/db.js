...
module.exports = {
  ...
  saveTemplate(userId, tpl) {
    ...
    // Propagate gig-window flags to existing weddings that use this template
    // (matched by questionnaire name). This keeps older snapshots in sync so the
    // gig window shows the right questions without re-applying the template.
    const byLabel = {};
    clean.questions.forEach(q => { if (q.label) byLabel[q.label.trim().toLowerCase()] = q; });
    store.weddings.forEach(w => {
      if (w.host_id !== userId) return;
      const q = w.questionnaire;
      if (!q || !Array.isArray(q.questions)) return;
      if ((q.name || '').trim().toLowerCase() !== (clean.name || '').trim().toLowerCase()) return;
      let changed = false;
      q.questions.forEach(wq => {
        // Match by id first, then by label (older snapshots may differ).
        const src = clean.questions.find(cq => cq.id === wq.id) || byLabel[(wq.label || '').trim().toLowerCase()];
        if (src && !!wq.gigShow !== !!src.gigShow) { wq.gigShow = !!src.gigShow; changed = true; }
        else if (src && wq.gigShow === undefined && src.gigShow) { wq.gigShow = true; changed = true; }
      });
      if (changed) w._q_synced = Date.now();
    });
    persist();
    return clean;
  },

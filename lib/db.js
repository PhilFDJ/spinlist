...
try {
  ...
  store = { users: [], sessions: [], processed_events: [], events_created: [], codes: [], redemptions: [], events: [] };
}
// ensure all collections exist even if an older file is loaded
for (const k of ['users', 'sessions', 'processed_events', 'events_created', 'codes', 'redemptions', 'events']) {
...
module.exports = {
  ...
  createEvent(e) {
    const event = {
      ...
      locked: e.locked ? 1 : 0,
      archived: 0,
      tracks: {},                 // trackId -> { id, uri, title, artist, art, votes }
  ...
  // `remove` = array of trackIds to -1. Returns the updated event.
  applyVotes(id, { add = [], remove = [] }) {
    const e = this.getEvent(id);
    if (!e) return null;
    for (const t of add) {
      if (!e.tracks[t.id]) {
        e.tracks[t.id] = { id: t.id, uri: t.uri || null, title: t.title, artist: t.artist, art: t.art || '', votes: 0, played: 0, addedAt: Date.now() };
      }
